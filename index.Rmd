...
Students concern about grades all the time. Our report focuses on using data visualization to help us and every other UW student to understand grade disparity and to help us make better decisions.

Many students go to sites like [ratemyprofessor.com](ratemyprofessor.com) or UW’s [course evaluation](https://www.washington.edu/cec/toc.html) to learn more about the courses before they commit. However, many sources cannot provide a macro overview. Therefore, we set out to help to leverage macro data into course planning. By visualizing the average GPA, grade distributions of UW courses, students can better assess the difficulties of each class they are taking. It also helps to foster transparency across college courses and would help us to make informed decisions to choose the right schedule for us.

The dataset we are using comes from the UW public record office. It includes the past eight years of courses taken place in the Seattle campus, with letter grades, average GPA, professor ratings, etc. We believe this dataset could answer many questions but we will mostly focus on undergraduate INFO classes for this report.

## Summary
<<<<<<< HEAD
Influences on a student’s decision to choose a particular course to study have been popular topics of researcher for many educators. For this report, we gather some general information that a student will need when deciding his/her enrollment. We are only concerned with class within Info department in this report. As indicated by the dataset, the biggest class in student enrollment is `r biggest_class`. `r most_wf` is the class that has the greatest number of Drop/Fail. `r lowest_gpa` is the info class that has the lowest average class gpa, while the class with most 4.0 in percentage is `r most_four_course`. Additionally, Professor rating is also an important factor in deciding which course to take. We also find out the top 10 professor names with highest rating: `r data_with_pros`.
=======

Influences on a student’s decision to choose a particular course to study have been popular topics of researcher for many educators. For this report, we gather some general information that a student will need when deciding his/her enrollment. We are only concerned with class within Info department in this report. As indicated by the dataset, the biggest class in student enrollment is ‘r biggest_class’. ‘R most_wf’ is the class that has the greatest number of Drop/Fail. ‘R lowest_gpa’ is the info class that has the lowest average class gpa. Additionally, Professor rating is also an important factor in deciding which course to take. We also find out the top 10 professor names with highest rating: ‘r data_with_pros’
>>>>>>> 8f44d3a6df91d846c19bb96ae70d3db49328a5e2
...
```{r echo=FALSE}
kable(P_table,col.names = c("Course", "Title", "Class Popularity"))
```
